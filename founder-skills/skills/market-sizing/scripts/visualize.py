...
# Skill scripts are standalone and cannot import across files, so the shared-input detector
# and the two helpers it needs live here too. test_visualize_market_sizing.py parses both
# files and compares these bodies, so a drift reds rather than shipping two surfaces that
# disagree about what the founder is told.
...
def _chart_adversarial(redteam: dict[str, Any] | None) -> str:
    ...
    parts: list[str] = []
    if not findings:
        parts.append(f"<p>{_esc(_adversarial_outcome(redteam, None))} That is a result, not an absence of one.</p>")
    else:
        parts.append(f"<p>{_esc(_adversarial_outcome(redteam, None))} Each quotes the sentence it relies on.</p>")
        for f in sorted(findings, key=lambda x: _SEVERITY_ORDER.get(str(x.get("severity")), 3)):
            claim = _esc(_humanize_claim(str(f.get("claim_attacked") or "").strip()))
            truth = _esc(str(f.get("what_is_true") or "").strip())
            ...
            if url == _INTERNAL_PROVENANCE:
                ...
                m = re.match(r"^document:([^#]+)(?:#page=(\d+))?$", url)
                if not m:
                    cite = url[len("document:") :]
                else:
                    cite = f"{m.group(1)}, page {m.group(2)}" if m.group(2) else m.group(1)
    ...
    if unchecked:
        items = "".join(f"<li>{_esc(str(u).strip())}</li>" for u in unchecked)
...
def compose_html(dir_path: str) -> str:
    ...
    adversarial_html = _chart_adversarial(artifacts.get("redteam.json"))
    ...
    if _usable(sizing) and not _is_stub(sizing):
        ...
        verdict_html = f'<p class="verdict">{_esc(_verdict_text)}</p>'
    else:
        _outcome = _esc(_adversarial_outcome(_rt_for_verdict, _skip))
