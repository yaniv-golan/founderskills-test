...
# ///
"""Image-only PDF probe (per-page).

COPY of skills/cap-table/scripts/pdf_probe.py, kept byte-identical below the docstring. Skill
scripts are standalone and cannot import across skills, so a probe two skills need exists twice;
`tests/test_pdf_probe_sync.py` compares the parsed bodies, so a drift reds instead of leaving two
skills disagreeing about whether a founder's document is readable.

The RTL block is inert for market-sizing -- it is a cap-table-locale concern -- and is copied
anyway rather than trimmed: a partial copy makes the parity test assert a subset, which is the
weaker guard, and the block is warning-only (it never changes image_only / kind / the exit code).

Used by RED_TEAM (Step 6c). The red-team sub-agent has no Bash, so the MAIN THREAD probes before
dispatching and tells the agent which documents it cannot read reliably. Without that, the agent
reads an image-only PDF by vision, under-extracts silently, and reports "nothing found" about a
page it never really saw -- an unchecked claim presented as checked, which is the one failure this
