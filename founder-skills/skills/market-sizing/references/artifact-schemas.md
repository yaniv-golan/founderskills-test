...
Written by `red_team.py` from the RED_TEAM sub-agent's hand-off (Step 6c). Optional artifact; when absent, `methodology.red_team_skipped` must say why.
...
| `rejected[]` | object[] | `{claim_attacked, reason}` for each finding that could not be shown (no source, a document not supplied, a token quoted as a sentence). Counted, never dropped silently. |
...
| `summary` | object | `accepted`, `rejected`, `unchecked`, `sources_unread`, `by_severity`. |
